...
# ==========================
def home(request):
    # Live public-facing statistics for the homepage.
    total_users = User.objects.count()
    total_assessments = Patient.objects.count()
...
def quick_predict(request):
    ...
    if request.method == "POST":
        try:
            ...
            confidence = round(max(model.predict_proba(scaled_features)[0]) * 100, 2)
            prediction = "Diabetic" if result[0] == 1 else "Not Diabetic"

            return render(request, "public_result.html", {
                "name": name, "prediction": prediction, "confidence": confidence,
                "age": age, "glucose": glucose, "blood_pressure": blood_pressure,
                "bmi": bmi, "today": datetime.now(),
            })
        except (ValueError, KeyError):
            return render(request, "public_predict.html", {"error": "Please enter valid values in all fields."})

    return render(request, "public_predict.html")
...
def download_pdf(request, patient_id):
    ...
    story.append(Paragraph("GlucoSense Health Report", title))
    ...
    story.append(
        Paragraph(
            "<b>Generated by Smart Diabetes Screening & Patient Care Portal</b>",
            normal
        )
    )
...
# Public pages opened from Home page
